#!/usr/bin/env python
"""UTAG on cell-type indicator features — all TMAs, T-panel v8.
...
for res in RESOLUTIONS:
    ...
    ax.set_title(f"T-panel v8: UTAG Cell-Type Features (max_dist={MAX_DIST}, res={res})\n"
    ...
    ax.set_title(f"T-panel v8: Domain Distribution per TMA (res={res})", fontweight="bold")
    ax.set_ylabel("TMA")
...
# ---- FL11 spatial plot (res=0.3 and 0.5) ----
print("\n=== FL11 spatial plots ===")
...
if len(roi) > 0:
    ...
    plt.suptitle(f"UTAG Cell-Type Features — B1_FL11 (T-panel v8, max_dist={MAX_DIST})",
...
for res in RESOLUTIONS:
    ...
    if col in adata.obs.columns:
        ...
        n_fl11 = adata[adata.obs["sample_id"] == "B1_FL11"].obs[col].nunique()
