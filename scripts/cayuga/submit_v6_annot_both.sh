#!/bin/bash
#SBATCH --job-name=v6_annot
#SBATCH --output=logs/v6_annot_%j_%a.out
#SBATCH --error=logs/v6_annot_%j_%a.err
...
if [ "$PANEL" == "T" ]; then
    ...
    OUT=output/all_TMA_T_global_v6.h5ad
else
    CKPT=output/all_TMA_S_ckpt_v3_hires.h5ad
    OUT=output/all_TMA_S_global_v6.h5ad
fi
...
echo "Done: ${PANEL}-panel v6 annotation (harmonized gates)"
