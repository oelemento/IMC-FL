#!/usr/bin/env python
"""Run UTAG on all TMAs T-panel v8 (2.18M cells, 170 ROIs)."""
import time
...
if mid_col:
    ...
    sns.heatmap(ct, annot=True, fmt=".2f", cmap="YlOrRd", ax=ax,
                ...
                annot_kws={"size": 6})
    ax.set_title(f"T-panel v8: Cell Type Composition per UTAG Domain\n({mid_col})", fontweight="bold")
...
if mid_col:
    ...
    ax.set_title(f"T-panel v8: UTAG Domain Distribution per TMA\n({mid_col})", fontweight="bold")
    ax.set_ylabel("TMA")
...
if mid_col:
    ...
    ax.set_title(f"T-panel v8: UTAG Domain Distribution per ROI\n({mid_col})", fontweight="bold")
    ax.set_ylabel("ROI")
    ax.set_xlabel("UTAG Domain")
    plt.tight_layout()
