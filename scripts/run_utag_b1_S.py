#!/usr/bin/env python
"""Run UTAG on full B1 S-panel (631K cells, all 50 ROIs together)."""
import time
...
INPUT = f"{OUTDIR}/batch_S/B1_S_raw_combined.h5ad"

print("=== Loading B1 S-panel data ===")
t0 = time.time()
...
# Save
outpath = f"{OUTDIR}/B1_S_utag_full.h5ad"
...
for col in utag_cols:
    ...
    fig.savefig(f"{OUTDIR}/B1_S_utag_spatial_{res_tag}.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved B1_S_utag_spatial_{res_tag}.png")
...
if mid_col in utag_results.obs.columns:
    ...
    ax.set_xlabel("")
    plt.tight_layout()
    fig.savefig(f"{OUTDIR}/B1_S_utag_composition.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    print("  Saved B1_S_utag_composition.png")
...
if mid_col in utag_results.obs.columns:
    ...
    plt.tight_layout()
    fig.savefig(f"{OUTDIR}/B1_S_utag_roi_domains.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    print("  Saved B1_S_utag_roi_domains.png")
