#!/usr/bin/env python3
"""Wendy EZH2 mut vs WT — slide 7 fix: B cells close vs distant from MACROPHAGES.

Her actual question: "Are Mut EZH2 B cells CLOSE TO MACROPHAGES higher for ki67
and survival signals?" My earlier slide 7 used CD14+ FDCs as the proximity
anchor (paper's existing design). This version uses macrophage cell types as
the anchor, as she asked.

Macrophage anchor set = ['M1 Macrophages', 'M2 Macrophages', 'Macrophages',
                         'Histiocytes (CD44hi)']  (S-panel myeloid cells)

For each follicular B cell (B cells (BCL2+) or B cells (PAX5+)) in each ROI,
compute distance to the nearest macrophage. Close = <=30 um, distant = >30 um.
Compare Ki-67 (mean per patient) close vs distant, within each EZH2 group
(paired Wilcoxon) and between groups (MW).

Outputs:
  output/ezh2/b_cells_near_mac/fig_ezh2_b_cells_near_mac.png
  output/ezh2/b_cells_near_mac/ki67_close_vs_distant_mac_per_patient.csv
"""
import argparse, sys
...
MACROPHAGE_TYPES = ["M1 Macrophages", "M2 Macrophages", "Macrophages",
                     "Histiocytes (CD44hi)"]
...
def main():
    ...
    ap.add_argument("--out", default="output/ezh2/b_cells_near_mac")
    args = ap.parse_args()
    ...
    for sid, sub in df.groupby("sid_norm"):
        mac = sub[sub.cell_type.isin(MACROPHAGE_TYPES)]
        if len(mac) < MIN_MAC_PER_ROI:
            continue
        b_sub = sub[sub.cell_type.isin(FOLLICULAR_B)]
        if len(b_sub) < 50:
            continue
        mac_coords = np.column_stack([mac.cx.values, mac.cy.values])
        ...
        rows.append({
            ...
            "ki67_close_mean": float(b_sub.Ki67.values[close_mask].mean()),
            "ki67_distant_mean": float(b_sub.Ki67.values[~close_mask].mean()),
            "delta_close_minus_distant": float(b_sub.Ki67.values[close_mask].mean()
                                                - b_sub.Ki67.values[~close_mask].mean()),
        })
    roi_df = pd.DataFrame(rows)
    print(f"  ROIs with B cells + macrophages and both close+distant: {len(roi_df)} "
          f"({roi_df.EZH2.value_counts().to_dict()})")

    metric_cols = ["ki67_close_mean", "ki67_distant_mean", "delta_close_minus_distant",
                   "n_close", "n_distant", "n_macrophages"]
    pt_df = roi_df.groupby(["Patient_ID", "EZH2"])[metric_cols].mean().reset_index()
    pt_df.to_csv(out_dir / "ki67_close_vs_distant_mac_per_patient.csv", index=False)
    ...
    p_wt_wilc = wilcoxon_test(pt_df.loc[pt_df.EZH2 == "wt", "delta_close_minus_distant"].values)
    p_mut_wilc = wilcoxon_test(pt_df.loc[pt_df.EZH2 == "mut", "delta_close_minus_distant"].values)
    p_close = mw_test(pt_df.loc[pt_df.EZH2 == "wt", "ki67_close_mean"].values,
                       pt_df.loc[pt_df.EZH2 == "mut", "ki67_close_mean"].values)
    p_distant = mw_test(pt_df.loc[pt_df.EZH2 == "wt", "ki67_distant_mean"].values,
    ...
    print(f"\n  Wilcoxon close-vs-distant within WT (paired): p={p_wt_wilc:.4g}")
    print(f"  Wilcoxon close-vs-distant within Mut (paired): p={p_mut_wilc:.4g}")
    print(f"  MW WT-vs-Mut on close cells:    p={p_close:.4g}")
    print(f"  MW WT-vs-Mut on distant cells:  p={p_distant:.4g}")
    ...
    ax.set_title(
        f"(a) B cell Ki-67 near vs distant from macrophages, by EZH2\n"
        f"≤{int(DIST_THRESHOLD_UM)} µm = close; macrophage anchor = M1/M2/Mac/Histiocytes",
        fontsize=10,
    )
    for sp in ("top", "right"):
    ...
    fig.suptitle("EZH2 Mut vs WT — B cell proliferation near macrophages (S-panel IMC)",
                 fontsize=13, y=1.03)
    plt.tight_layout()
    out = out_dir / "fig_ezh2_b_cells_near_mac.png"
