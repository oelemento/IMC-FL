#!/usr/bin/env python
"""UTAG per-TMA + meta-clustering for S-panel v8."""
import time
...
# 2. Dendrogram + composition heatmap
fig, (ax_dend, ax_heat) = plt.subplots(1, 2, figsize=(22, max(12, len(ct_comp) * 0.15)),
...
ax.set_title(f"S-panel v8: Tissue Compartment Distribution per TMA", fontweight="bold")
ax.set_ylabel("TMA")
...
if len(roi) > 0:
    ...
    ct_colors = {
        "B (BCL2+)": "#FFB347", "B (PAX5+)": "#FFA07A", "B": "#FFDEAD",
        "CD4 T": "#9370DB", "CD8 T": "#90EE90", "FDC": "#FF4500",
        "FRC": "#8B4513", "M1 Mac": "#228B22", "M2 Mac": "#006400",
        "Mac": "#32CD32", "DC": "#DAA520", "Stromal/CAF": "#A0522D",
        "Endothelial": "#4682B4", "Myeloid (S100A9+)": "#BDB76B",
        "Histiocytes": "#556B2F", "LQ": "#D3D3D3", "Mixed": "#A9A9A9",
        "Other": "#C0C0C0",
    }
    ...
    plt.suptitle("UTAG Per-TMA + Meta-clustering — B1_FL11 (S-panel v8)",
